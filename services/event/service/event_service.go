...
/*
Returns the list of all events and their attendees
*/
func GetAllEventsAndAttendees() ([]models.EventAttendees, error) {
	events, err := GetAllEvents[models.EventDB]()
	if err != nil {
		return nil, err
	}

	eventList := []models.EventAttendees{}

	for _, event := range events.Events {
		attendees, err := GetEventAttendees(event.ID)
		if err != nil {
			return nil, err
		}

		eventAttendees := models.EventAttendees{
			Event:     event.ID,
			Attendees: attendees,
		}

		eventList = append(eventList, eventAttendees)
	}
...
/*
Returns the list of attendees for the event with the given id
*/
func GetEventAttendees(eventID string) ([]string, error) {
	tracker, err := GetEventTracker(eventID)
	if err != nil {
		return nil, err
	}

	return tracker.Users, nil
}
